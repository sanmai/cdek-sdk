...
В других случаях тип возвращаемного значения может быть не определён в сигнатуре метода, а значит вместо исключения метод может вернуть `null`.
...
Подобное расхождение в обработке отсутствующих значений существует в силу исторических причин, устранить которое невозможно без частичной потери обратной совместимости (замена строгого возвращаемого типа на опциональный нарушает обратную совместимость). Эти различия могут быть частично или полностью устранены заменой на возврат опционального значения в следующей мажорной версии, о чём будет обязательно сообщено.
